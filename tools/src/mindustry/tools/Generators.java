...
public class Generators {
    ...
    private static float fluid(boolean gas, double x, double y, float frame) {
        ...
        if (gas) {
            float min = 0.56f;
            float interpolated = Mathf.lerp((float) gasFrame(x, y, curFrame), (float) gasFrame(x, y, nextFrame),
                    progress);
    ...
    private static double gasFrame(double x, double y, int frame) {
        ...
        return (Simplex.rawTiled(x, y, ox, oy, s, s, scale)
                + Simplex.rawTiled(x, y, ox, oy, s, s, scale / 1.5) * second) / (1.0 + second);
    }
    ...
    public static void run() {
                        ...
                        for (int y = 0; y < output.height; y++) {
                            ...
                            for (int cx = -rad; cx < rad; cx++) {
                                for (int cy = -rad; cy < rad; cy++) {
                                    int wx = Mathf.clamp(cx + x, 0, output.width - 1),
                                            wy = Mathf.clamp(cy + y, 0, output.height - 1);
        ...
        generate("block-icons", () -> {
            ...
            for (Block block : content.blocks()) {
                if (block.isAir() || block instanceof ConstructBlock || block instanceof OreBlock
                        || block instanceof LegacyBlock)
                    continue;
                    ...
                    for (Team team : Team.all) {
                        if (team.hasPalette) {
                            ...
                            teamr.each((x, y) -> {
                                ...
                                int index = color == 0xffffffff ? 0
                                        : color == 0xdcc6c6ff ? 1 : color == 0x9d7f7fff ? 2 : -1;
                ...
                try {
                    ...
                    if (block.outlineIcon) {
                        GenRegion region = (GenRegion) regions[block.outlinedIcon >= 0 ? block.outlinedIcon
                                : regions.length - 1];
                        Pixmap base = get(region);
                ...
                for (TextureRegion region : floor.floorBase.variantRegions()) {
                    ...
                    for (int x = 0; x < res.width; x++) {
                        for (int y = 0; y < res.height; y++) {
                            res.set(x, y,
                                    Pixmap.blend(
                                            (overlay.getRaw(x, y) & 0xffffff00) | (int) (floor.liquidOpacity * 255),
                                            res.getRaw(x, y)));
                        }
                    }
        ...
        generate("unit-icons", () -> content.units().each(type -> {
            if (type.internal)
                return; // internal hidden units don't generate
            ...
            try {
                ...
                for (Weapon weapon : weapons) {
                    if (outlined.add(weapon.name) && has(weapon.name)) {
                        // only non-top weapons need separate outline sprites (this is mostly just
                        // mechs)
                ...
                if (sample instanceof Tankc) {
                    ...
                    for (int r = 0; r < type.treadRects.length; r++) {
                        ...
                        Pixmap slice = pix.crop((int) (treadRect.x + pix.width / 2f),
                                (int) (treadRect.y + pix.height / 2f), 1, (int) treadRect.height);
                ...
                Func<Weapon, Pixmap> weaponRegion = weapon -> Core.atlas.has(weapon.name + "-preview")
                        ? get(weapon.name + "-preview")
                        : get(weapon.region);
                Cons2<Weapon, Pixmap> drawWeapon = (weapon, pixmap) -> image.draw(
                        weapon.flipSprite ? pixmap.flipX() : pixmap,
                ...
                // outline is currently never needed, although it could theoretically be
                // necessary
                ...
                if (type.drawCell) {
                    ...
                    cell.replace(in -> in == 0xffffffff ? 0xffa664ff
                            : in == 0xdcc6c6ff || in == 0xdcc5c5ff ? 0xd06b53ff : 0);
                ...
                for (Weapon weapon : weapons) {
                    ...
                    if (weapon.cellRegion.found()) {
                        ...
                        weaponCell.replace(in -> in == 0xffffffff ? 0xffa664ff
                                : in == 0xdcc6c6ff || in == 0xdcc5c5ff ? 0xd06b53ff : 0);
                ...
                Vec2 offset = new Vec2(1, 1).rotate(rand.random(360f)).setLength(rand.random(0, offsetRange))
                        .add(image.width / 2f, image.height / 2f);
                ...
                image.each((x, y) -> {
                    ...
                    int section = (int) Mathf.clamp(
                            Mathf.mod(offset.angleTo(x, y) + noise + degrees, 360f) / 360f * splits, 0, splits - 1);
                    if (!vval)
                        wrecks[section].setRaw(x, y, Color.muli(image.getRaw(x, y), rValue ? 0.7f : 1f));
                });
                ...
                try {
                    ...
                    for (int x = 0; x < edge.width; x++) {
                        for (int y = 0; y < edge.height; y++) {
                            result.set(x, y,
                                    Color.muli(edge.getRaw(x, y), image.get(x % image.width, y % image.height)));
                        }
                    }
    ...
    public static class ScorchGenerator {
        ...
        private double noise(float angle) {
            return Math.pow(Simplex.noise2d(seed, octaves, pers, 1 / scale, Angles.trnsx(angle, size / 2f) + size / 2f,
                    Angles.trnsy(angle, size / 2f) + size / 2f), pow);
        }
    }
