...
public interface Autotiler {
    ...
    default @Nullable int[] getTiling(BuildPlan req, Eachable<BuildPlan> list) {
        ...
        // TODO this is O(n^2), very slow, should use quadtree or intmap or something
        // instead
        list.each(other -> {
     ...
     *         [1]: X scale
     *         [2]: Y scale
     *         [3]: a 4-bit mask with bits 0-3 indicating blend state in that
     *         direction (0 being 0 degrees, 1 being 90, etc)
    ...
    default int[] buildBlending(Tile tile, int rotation, BuildPlan[] directional, boolean world) {
        ...
        int num = (blends(tile, rotation, directional, 2, world) && blends(tile, rotation, directional, 1, world)
                && blends(tile, rotation, directional, 3, world))
                        ? 0
                        : (blends(tile, rotation, directional, 1, world)
                                && blends(tile, rotation, directional, 3, world))
                                        ? 1
                                        : (blends(tile, rotation, directional, 1, world)
                                                && blends(tile, rotation, directional, 2, world))
                                                        ? 2
                                                        : (blends(tile, rotation, directional, 3, world)
                                                                && blends(tile, rotation, directional, 2, world))
                                                                        ? 3
                                                                        : blends(tile, rotation, directional, 1, world)
                                                                                ? 4
                                                                                : blends(tile, rotation, directional, 3,
                                                                                        world) ? 5 : -1;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (blends(tile, rotation, directional, i, world) && (tile != null && tile.nearbyBuild(realDir) != null
                    && !tile.nearbyBuild(realDir).block.squareSprite)) {
                blendresult[4] |= (1 << i);
            }
        }
    ...
    default boolean blends(Tile tile, int rotation, @Nullable BuildPlan[] directional, int direction,
            boolean checkWorld) {
    ...
    default boolean blends(Tile tile, int rotation, int direction) {
        ...
        return other != null && other.team == tile.team()
                && blends(tile, rotation, other.tileX(), other.tileY(), other.rotation, other.block);
    }
    ...
    default boolean blendsArmored(Tile tile, int rotation, int otherx, int othery, int otherrot, Block otherblock) {
        return Point2.equals(tile.x + Geometry.d4(rotation).x, tile.y + Geometry.d4(rotation).y, otherx, othery)
                || ((!otherblock.rotatedOutput(otherx, othery)
                        ...
                        (otherblock.rotatedOutput(otherx, othery) && Point2.equals(otherx + Geometry.d4(otherrot).x,
                                othery + Geometry.d4(otherrot).y, tile.x, tile.y)));
    }
    ...
    default boolean notLookingAt(Tile tile, int rotation, int otherx, int othery, int otherrot, Block otherblock) {
        return !(otherblock.rotatedOutput(otherx, othery)
                && Point2.equals(otherx + Geometry.d4(otherrot).x, othery + Geometry.d4(otherrot).y, tile.x, tile.y));
    }

    /**
     * @return whether this tile is looking at the other tile, or the other tile is
     *         looking at this one.
     *         If the other tile does not rotate, it is always considered to be
     *         facing this one.
     */
