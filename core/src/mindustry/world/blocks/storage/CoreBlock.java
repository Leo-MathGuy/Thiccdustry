...
public class CoreBlock extends StorageBlock {
    protected static final float cloudScaling = 1700f, cfinScl = -2f, cfinOffset = 0.3f, calphaFinOffset = 0.25f,
            cloudAlpha = 0.81f;
    ...
    public static void playerSpawn(Tile tile, Player player) {
        if (player == null || tile == null || !(tile.build instanceof CoreBuild core))
            return;
    ...
    public void setBars() {
        ...
        addBar("capacity", (CoreBuild e) -> new Bar(
                ...
                () -> Pal.items,
                () -> e.items.total()
                        / ((float) e.storageCapacity * content.items().count(UnlockableContent::unlockedNow))));

        addBar("power", (CoreBuild e) -> new Bar(() -> Core.bundle.format("bar.poweroutput",
                Strings.fixed(e.getPowerProduction() * 60 * e.timeScale(), 1)),
                () -> Pal.powerBar,
                () -> 1f));
    }
    ...
    public void beforePlaceBegan(Tile tile, Block previous) {
        if (tile.build instanceof CoreBuild) {
            // right before placing, create a "destination" item array which is all the
            // previous items minus core requirements
            ...
            drawPlaceText(Core.bundle.get(
                    ...
                    // TODO better message
                            ...
                            : (player.team().core() != null
                                    && player.team().core().items.has(requirements, state.rules.buildCostMultiplier))
                                    || state.rules.infiniteResources ? "bar.corereq" : "bar.noresources"),
                    x, y, valid);
        }
    }
    ...
    public class CoreBuild extends Building {
        ...
        // utility methods for less Block-to-CoreBlock casts. also allows for more
        // customization
        ...
        public void beginLaunch(@Nullable CoreBlock launchType) {
            ...
            if (!headless) {
                ...
                if (renderer.isLaunching()) {
                    ...
                    image.actions(Actions.delay((landDuration() - margin) / 60f),
                            Actions.fadeIn(margin / 60f, Interp.pow2In), Actions.delay(6f / 60f), Actions.remove());
                    image.update(() -> {
                        image.toFront();
                        ui.loadfrag.toFront();
                        if (state.isMenu()) {
                    ...
                    Time.run(landDuration(), () -> {
                        ...
                        if (state.isCampaign() && Vars.showSectorLandInfo && (state.rules.sector.preset == null
                                || state.rules.sector.preset.showSectorLandInfo)) {
                            ui.announce("[accent]" + state.rules.sector.name() + "\n" +
                                    (state.rules.sector.info.resources.any()
                                            ? "[lightgray]" + Core.bundle.get("sectors.resources") + "[white] " +
                                                    state.rules.sector.info.resources.toString(" ",
                                                            UnlockableContent::emoji)
                                            : ""),
                                    5);
                        }
                    });
        ...
        public void drawLanding(CoreBlock block) {
            ...
            if (state.rules.cloudColor.a > 0.0001f) {
                ...
                Draw.rect(Tmp.tr1, Core.camera.position.x, Core.camera.position.y, Core.camera.width,
                        Core.camera.height);
                Draw.reset();
            }
        }
        ...
        /**
         * @return Camera zoom while landing or launching. May optionally do other
         *         things such as setting camera position to itself.
         */
            ...
            if (renderer.getLandTime() >= 1f) {
                tile.getLinkedTiles(t -> {
                    if (Mathf.chance(0.4f)) {
                        Fx.coreLandDust.at(t.worldx(), t.worldy(), angleTo(t.worldx(), t.worldy()) + Mathf.range(30f),
        ...
        public void onDestroyed() {
            ...
            // add a spawn to the map for future reference - waves should be disabled, so it
            // shouldn't matter
            if (state.isCampaign() && team == state.rules.waveTeam && team.cores().size <= 1
                    && state.rules.sector.planet.enemyCoreSpawnReplace) {
                // do not recache
        ...
        public void drawSelect() {
            ...
            if (team.cores().size <= 1 && !proximity.contains(storage -> storage.items == items))
                return;
        ...
        public boolean owns(Building core, Building tile) {
            return tile instanceof StorageBuild b && ((StorageBlock) b.block).coreMerge
                    && (b.linkedCore == core || b.linkedCore == null);
        }
