...
@Component(base = true)
abstract class BuildingComp implements Posc, Teamc, Healthc, Buildingc, Timerc, QuadTreeObject, Displayable, Sized,
        Senseable, Controllable, Settable {
    ...
    /**
     * TODO Each bit corresponds to a team ID. Only 64 are supported. Does not work
     * on servers.
     */
    transient long visibleFlags;
    transient boolean wasVisible; // used only by the block renderer when fog is on (TODO replace with discovered
                                  // check?)
    ...
    public final void writeBase(Writes write) {
        ...
        write.b(enabled ? 1 : 0);
        // write presence of items/power/liquids/cons, so removing/adding them does not
        // corrupt future saves.
    ...
    public void addPlan(boolean checkPrevious, boolean ignoreConditions) {
        if (!ignoreConditions && (!block.rebuildable
                || (team == state.rules.defaultTeam && state.isCampaign() && !block.isVisible())))
            return;
        ...
        if (self() instanceof ConstructBuild entity) {
            ...
            if (entity.current != null && entity.current.synthetic() && entity.wasConstructing) {
                ...
                // otherwise this was a deconstruction that was interrupted, don't want to
                // rebuild that
                return;
            }
        }
        ...
        data.plans.addFirst(new BlockPlan(tile.x, tile.y, (short) rotation, toAdd.id,
                overrideConfig == null ? config() : overrideConfig));
    }
    ...
    public float calculateHeat(float[] sideHeat, @Nullable IntSet cameFrom) {
        ...
        for (var build : proximity) {
            if (build != null && build.team == team && build instanceof HeatBlock heater) {
                ...
                // non-routers must face us, routers must face away - next to a redirector,
                // they're forced to face away due to cycles anyway
    ...
    /**
     * Any class that overrides this method and changes the value must call
     * Vars.fogControl.forceUpdate(team).
     */
     ...
     *         usually used for crafters and things that need to spin up before
    ...
    /**
     * @return total time this block has been producing something; non-crafter
     *         blocks usually return Time.time.
     */
    ...
    public boolean allowUpdate() {
        ...
        // check if outside map limit
                (!state.rules.limitMapArea || !state.rules.disableOutsideArea || Rect.contains(state.rules.limitX,
                        state.rules.limitY, state.rules.limitWidth, state.rules.limitHeight, tile.x, tile.y));
    }
    ...
    /**
     * Call when nothing is happening to the entity. This increments the internal
     * sleep timer.
     */
    public void sleep() {
    ...
    /**
     * @return whether the player can select (but not actually control) this
     *         building.
     */
    ...
    /**
     * Called when a player control-selects this building - not called for
     * ControlBlock subclasses.
     */
    ...
    /**
     * Called when this building receives a position command. Requires a commandable
     * block.
     */
    ...
    public void handleUnitPayload(Unit unit, Cons<Payload> grabber) {
        ...
        if (net.client()) {
            ...
            // server-side, this needs to be delayed until next frame because otherwise the
            // packets sent out right after this event would have the wrong unit ID, leading
            // to ghosts
    ...
    /**
     * @return whether this block is currently "active" and should be consuming
     *         requirements.
     */
    ...
    /**
     * @return whether this building is currently "burning" a trigger consumer (an
     *         item) - if true, valid() on those will return true.
     */
    ...
    /**
     * @param outputDir output liquid direction relative to rotation, or -1 to use
     *                  any direction.
     */
    ...
    public float moveLiquid(Building next, Liquid liquid) {
        ...
        if (next.team == team && next.block.hasLiquids && liquids.get(liquid) > 0f) {
            ...
            if (flow > 0f && ofract <= fract && next.acceptLiquid(self(), liquid)) {
                ...
                // TODO !IMPORTANT! uses current(), which is 1) wrong for multi-liquid blocks
                // and 2) causes unwanted reactions, e.g. hydrogen + slag in pump
    ...
    /**
     * Tries to put this item into a nearby container. Returns success. Unlike
     * #offload(), this method does not change the block inventory.
     */
    ...
    /**
     * Dumps any item with an accumulator. May dump multiple times per frame. Use
     * with care.
     */
    ...
    /**
     * Try offloading an item to a nearby container in its facing direction. Returns
     * true if success.
     */
    ...
    /**
     * Called after this building is created in the world. May be called multiple
     * times, or when adjacent buildings change.
     */
    ...
    /**
     * Called when anything adjacent to this building is placed/removed, including
     * itself.
     */
    ...
    public Seq<Building> getPowerConnections(Seq<Building> out) {
        ...
        for (Building other : proximity) {
            if (other != null && other.power != null
                    && other.team == team
                    && !(block.consumesPower && other.block.consumesPower && !block.outputsPower
                            && !other.block.outputsPower && !block.conductivePower && !other.block.conductivePower)
    ...
    public void drawCracks() {
        if (!block.drawCracks || !damaged() || block.size > BlockRenderer.maxCrackSize)
            return;
        int id = pos();
        TextureRegion region = renderer.blocks.cracks[block.size - 1][Mathf
                .clamp((int) ((1f - healthf()) * BlockRenderer.crackRegions), 0, BlockRenderer.crackRegions - 1)];
    ...
    public void draw() {
        if (block.variants == 0 || block.variantRegions == null) {
            ...
            Draw.rect(
                    block.variantRegions[Mathf.randomSeed(tile.pos(), 0, Math.max(0, block.variantRegions.length - 1))],
                    x, y, drawrot());
        }
    ...
    public void drawLight() {
        ...
        if (block.hasLiquids && block.drawLiquidLight && liq.lightColor.a > 0.001f) {
            // yes, I am updating in draw()... but this is purely visual anyway, better have
            // it here than in update() where it wastes time
    ...
    /**
     * @return whether a building has regen/healing suppressed; if so, spawns
     *         particles on it.
     */
    public boolean checkSuppression() {
        if (isHealSuppressed()) {
            if (Mathf.chanceDelta(0.03)) {
                Fx.regenSuppressParticle.at(x + Mathf.range(block.size * tilesize / 2f - 1f),
                        y + Mathf.range(block.size * tilesize / 2f - 1f), suppressColor);
            }
    ...
    /**
     * Called when a block is placed over some other blocks. This seq will always
    ...
    public void configured(@Nullable Unit builder, @Nullable Object value) {
        ...
        Class<?> type = value == null ? void.class
                : value.getClass().isAnonymousClass() ? value.getClass().getSuperclass() : value.getClass();
    ...
    public void afterDestroyed() {
        if (block.destroyBullet != null) {
            ...
            block.destroyBullet.create(this, block.destroyBulletSameTeam ? team : Team.derelict, x, y,
                    Mathf.randomSeed(id(), 360f));
        }
    }
    ...
    /**
     * Called when a block begins (not finishes!) deconstruction. The building is
    ...
    public void onDeconstructed(@Nullable Unit builder) {
        ...
        if (liquids != null && liquids.currentAmount() > 0
                && (!liquids.current().incinerable || block.deconstructDropAllLiquid)) {
    ...
    public void onDestroyed() {
        ...
        if (block.hasLiquids && state.rules.damageExplosions) {
            ...
            liquids.each((liquid, amount) -> {
                ...
                for (int i = 0; i < Mathf.clamp(amount / 5, 0, 30); i++) {
                    Time.run(i / 2f, () -> {
                        Tile other = world.tileWorld(x + Mathf.range(block.size * tilesize / 2),
                                y + Mathf.range(block.size * tilesize / 2));
                        if (other != null) {
        ...
        Damage.dynamicExplosion(x, y, flammability, explosiveness * 3.5f, power, tilesize * block.size / 2f,
                state.rules.damageExplosions, block.destroyEffect);
    ...
    public void display(Table table) {
        ...
        if (team == player.team()) {
            ...
            boolean displayFlow = (block.category == Category.distribution || block.category == Category.liquid)
                    && block.displayFlow;
                        ...
                        Runnable rebuild = () -> {
                            ...
                            for (Item item : content.items()) {
                                if (flowItems.hasFlowItem(item)) {
                                    ...
                                    l.label(() -> flowItems.getFlowRate(item) < 0 ? "..."
                                            : Strings.fixed(flowItems.getFlowRate(item), 1) + ps)
                        ...
                        Runnable rebuild = () -> {
                            ...
                            for (var liquid : content.liquids()) {
                                if (liquids.hasFlowLiquid(liquid)) {
                                    ...
                                    l.label(() -> liquids.getFlowRate(liquid) < 0 ? "..."
                                            : Strings.fixed(liquids.getFlowRate(liquid), 1) + ps)
                                            .color(Color.lightGray);
                                    l.row();
                                }
                            }
     ...
     *
     * @return whether the tap event is consumed - if true, the player will not
     *         start shooting or interact with things under the cursor.
     */
    ...
    /**
     * Returns whether this config menu should show when the specified player taps
     * it.
     */
    ...
    /**
     * Whether this configuration should be hidden now. Called every frame the
     * config is open.
     */
    ...
    /**
     * Called after efficiency is updated but before consumers are updated. Use to
     * apply your own multiplier.
     */
    ...
    /**
     * Calculate your own efficiency multiplier. By default, this is applied in
    ...
    public double sense(LAccess sensor) {
        return switch (sensor) {
            ...
            // totalLiquids is inherently bad design, but unfortunately it is useful for
            // conduits/tanks
            case totalLiquids -> liquids == null ? 0 : liquids.currentAmount();
            case totalPower -> power == null || block.consPower == null ? 0
                    : power.status * (block.consPower.buffered ? block.consPower.capacity : 1f);
            ...
            case cameraX, cameraY, cameraWidth, cameraHeight ->
                this instanceof ControlBlock c ? c.unit().sense(sensor) : 0;
    ...
    public Object senseObject(LAccess sensor) {
        return switch (sensor) {
            ...
            case payloadType -> getPayload() instanceof UnitPayload p1 ? p1.unit.type
                    : getPayload() instanceof BuildPayload p2 ? p2.block() : null;
            default -> noSensed;
        };
    }
    ...
    public void control(LAccess type, Object p1, double p2, double p3, double p4) {
        // don't execute configure instructions that copy logic building configures;
        // this can cause extreme lag
    ...
    public void setProp(UnlockableContent content, double value) {
        if (content instanceof Item item && items != null) {
            ...
            if (amount < liquids.get(liquid) || (acceptLiquid(self(), liquid) && (liquids.current() == liquid
                    || liquids.currentAmount() <= 0.1f || block.consumesLiquid(liquid)))) {
