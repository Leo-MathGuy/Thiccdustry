...
public class Weapon implements Cloneable {
    ...
    /**
     * whether to create a flipped copy of this weapon upon initialization. default:
     * true
     */
    public boolean mirror = true;
    /**
     * whether to flip the weapon's sprite when rendering. internal use only - do
     * not set!
     */
    public boolean flipSprite = false;
    /**
     * whether to shoot the weapons in different arms one after another, rather than
     * all at once; only valid when mirror = true
     */
    ...
    /**
     * whether to hold the bullet in place while firing; it will still require
     * reload.
     */
    public boolean continuous;
    /**
     * whether this weapon uses continuous fire without reloading; implies
     * continuous = true
     */
    public boolean alwaysContinuous;
    /**
     * Speed at which the turret can change its bullet "aim" distance. This is only
     * used for point laser bullets.
     */
    ...
    /**
     * whether to automatically target relevant units in update(); only works when
     * controllable = false.
     */
    ...
    /**
     * time taken for weapon to return to starting position in ticks. uses reload
     * time by default
     */
    ...
    /**
     * should the shoot effects follow the unit (effects need followParent set to
    ...
    public void draw(Unit unit, WeaponMount mount) {
        ...
        if (parts.size > 0) {
            DrawPart.params.set(mount.warmup, mount.reload / reload, mount.smoothReload, mount.heat, mount.recoil,
                    mount.charge, wx, wy, weaponRotation + 90);
            DrawPart.params.sideMultiplier = flipSprite ? -1 : 1;

            for (int i = 0; i < parts.size; i++) {
                var part = parts.get(i);
                DrawPart.params
                        .setRecoil(part.recoilIndex >= 0 && mount.recoils != null ? mount.recoils[part.recoilIndex]
                                : mount.recoil);
                if (part.under) {
    ...
    public void update(Unit unit, WeaponMount mount) {
        ...
        if (recoils > 0) {
            if (mount.recoils == null)
                mount.recoils = new float[recoils];
        ...
        mount.charge = mount.charging && shoot.firstShotDelay > 0
                ? Mathf.approachDelta(mount.charge, 1, 1 / shoot.firstShotDelay)
                : 0;
        ...
        if (!controllable && autoTarget) {
            if ((mount.retarget -= Time.delta) <= 0f) {
                mount.target = findTarget(unit, mountX, mountY, bullet.range, bullet.collidesAir,
                        bullet.collidesGround);
            ...
            if (mount.target != null) {
                shoot = mount.target.within(mountX, mountY,
                        bullet.range + Math.abs(shootY) + (mount.target instanceof Sized s ? s.hitSize() / 2f : 0f))
                        && can;
        ...
        if (continuous && mount.bullet != null) {
            if (!mount.bullet.isAdded() || mount.bullet.time >= mount.bullet.lifetime || mount.bullet.type != bullet) {
                ...
                if (shootSound != Sounds.none && !headless) {
                    if (mount.sound == null)
                        mount.sound = new SoundLoop(shootSound, 1f);
        ...
        if (otherSide != -1 && alternate && mount.side == flipSprite && mount.reload <= reload / 2f
                && lastReload > reload / 2f) {
        ...
        if (mount.shoot && // must be shooting
                ...
                !(bullet.killShooter && mount.totalShots > 0) && // if the bullet kills the shooter, you should only
                                                                 // ever be able to shoot once
                (!useAmmo || unit.ammo > 0 || !state.rules.unitAmmo || unit.team.rules().infiniteAmmo) && // check ammo
                (!alternate || wasFlipped == flipSprite) &&
                mount.warmup >= minWarmup && // must be warmed up
                unit.vel.len() >= minShootVelocity && // check velocity requirements
                (mount.reload <= 0.0001f || (alwaysContinuous && mount.bullet == null)) && // reload has to be 0, or it
                                                                                           // has to be an
                                                                                           // always-continuous weapon
                (alwaysShooting || Angles.within(rotate ? mount.rotation : unit.rotation + baseRotation,
                        mount.targetRotation, shootCone)) // has to be within the cone
        ) {
    ...
    protected Teamc findTarget(Unit unit, float x, float y, float range, boolean air, boolean ground) {
        return Units.closestTarget(unit.team, x, y, range + Math.abs(shootY), u -> u.checkTarget(air, ground),
    ...
    protected float bulletRotation(Unit unit, WeaponMount mount, float bulletX, float bulletY) {
        return rotate ? unit.rotation + mount.rotation
                : Angles.angle(bulletX, bulletY, mount.aimX, mount.aimY)
                        + (unit.rotation - unit.angleTo(mount.aimX, mount.aimY)) + baseRotation;
    }
    ...
    protected void shoot(Unit unit, WeaponMount mount, float shootX, float shootY, float rotation) {
        ...
        shoot.shoot(mount.barrelCounter, (xOffset, yOffset, angle, delay, mover) -> {
            // this is incremented immediately, as it is used for total bullet creation
            // amount detection
            mount.totalShots++;
            ...
            if (delay > 0f) {
                Time.run(delay, () -> {
                    // hack: make sure the barrel is the same as what it was when the bullet was
                    // queued to fire
    ...
    protected void bullet(Unit unit, WeaponMount mount, float xOffset, float yOffset, float angleOffset, Mover mover) {
        ...
        float xSpread = Mathf.range(xRand),
                ...
                lifeScl = bullet.scaleLife
                        ? Mathf.clamp(Mathf.dst(bulletX, bulletY, mount.aimX, mount.aimY) / bullet.range)
                        : 1f,
                angle = shootAngle + Mathf.range(inaccuracy + bullet.inaccuracy);

        Entityc shooter = unit.controller() instanceof MissileAI ai ? ai.shooter : unit; // Pass the missile's shooter
                                                                                         // down to its bullets
        mount.bullet = bullet.create(unit, shooter, unit.team, bulletX, bulletY, angle, -1f,
                (1f - velocityRnd) + Mathf.random(velocityRnd), lifeScl, null, mover, mount.aimX, mount.aimY);
